...
export function MediaFilesEmptyState({ isLoadingList, onReloadTrackedImages, hasActiveSearch = false }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center">
      ...
      <h3 className="text-lg font-semibold text-slate-900">
        {hasActiveSearch ? "No media match your search" : "No media tracked yet"}
      </h3>
      <p className="mt-1 text-sm text-slate-600">
        {hasActiveSearch
          ? "Try a different name search or clear the search field."
          : "Load tracked entries to review your media data."}
      </p>
      {!hasActiveSearch ? (
        <Button
